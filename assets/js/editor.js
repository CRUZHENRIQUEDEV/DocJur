...
const DocJurEditor = (() => {
  ...
  /** @type {import("./utils.js").AppState} — subconjunto do state global;
   *  compartilhado por referência com DocJurApp via init(state).
   *  Não re-declare valores aqui — são inicializados no módulo pai. */
  let STATE = /** @type {import("./utils.js").AppState} */ (null);

  // ---- Referências DOM (avaliadas só após init) ----
  /** @type {{editor:HTMLElement|null,metaStats:HTMLElement|null,metaPh:HTMLElement|null}} */
  const refs = { editor: null, metaStats: null, metaPh: null, title: null, editorWrap: null };
  ...
  function applyPlaceholders() {
    ...
    const { html, filled, total } = replacePlaceholders(refs.editor.innerHTML, data);
    refs.editor.innerHTML = html;
    updateStats();
    if (refs.metaPh) refs.metaPh.textContent = `Placeholders: ${filled} preenchidos / ${total}`;
  }
  ...
  function loadTemplate(id) {
    ...
    const data = DocJurStore.collectData();
    const title = data.DOC_TITULO && String(data.DOC_TITULO).trim()
    ...
    applyPlaceholders();
    updateStats();
    if (window.DocJurUI) DocJurUI.toast(`Modelo carregado: ${DocJurTemplates.templateTitle(id)}`, "success");
  }
  ...
  function updateStats() {
    ...
    if (refs.metaStats) refs.metaStats.textContent = `Palavras: ${words} · Caracteres: ${chars}`;
  }

  function wordCountAlert() {
    const { words, chars, charsNoSpace } = countText(refs.editor?.innerText || "");
  ...
  function toggleLock() {
    ...
    refs.editor.setAttribute("contenteditable", STATE.editorLocked ? "false" : "true");
    ...
    if (window.DocJurUI) {
      DocJurUI.toast(STATE.editorLocked ? "Edição BLOQUEADA" : "Edição DESBLOQUEADA",
        STATE.editorLocked ? "warn" : "info");
    }
  }
  ...
  function initToolbar() {
    if (!STATE) throw new Error("[DocJurEditor] STATE não inicializado — chame Editor.init(state)");
    ...
    tbList?.addEventListener("change", (e) => {
      const sel = /** @type {HTMLSelectElement} */ (e.target);
      if (sel.value) document.execCommand(sel.value, false, null);
    ...
    $("#btn-unlink")?.addEventListener("click", () => {
      document.execCommand("unlink", false, null); refs.editor?.focus();
    });

    // --- Clear format, select all ---
    $("#btn-clear-fmt")?.addEventListener("click", () => {
      document.execCommand("removeFormat", false, null); refs.editor?.focus();
    });
    ...
    cc?.addEventListener("input", (e) => applyColor(/** @type {HTMLInputElement} */ (e.target).value));
    buildColorPalette();
    ...
    refs.editor?.addEventListener("keydown", (ev) => {
      ...
      else if (key === "u") { e.preventDefault(); document.execCommand("underline", false, null); }
    });

    // --- Zoom buttons ---
    $("#btn-zoom-in")?.addEventListener("click", () => setZoom(STATE.zoomLevel + 0.1));
    $("#btn-zoom-out")?.addEventListener("click", () => setZoom(STATE.zoomLevel - 0.1));
  ...
  function updateToolbarState() {
    ...
    cmds.forEach((c) => {
      const btn = /** @type {HTMLElement|null} */ (document.querySelector(`.tb-btn[data-cmd="${c}"]`));
  ...
  function insertPhAtCursor(key) {
    ...
    refs.editor?.focus();
    const snippet = `<span class="ph" data-ph="${escHtml(key)}">{{${escHtml(key)}}}</span>&nbsp;`;
  ...
  /**
   * @param {import("./utils.js").AppState} sharedState
   */
  ...
  return {
    init, applyPlaceholders, loadTemplate, updateStats, updateToolbarState,
    setZoom, toggleLock, insertPhAtCursor,
    get editorEl() { return refs.editor; }
  };
})();

window.DocJurEditor = DocJurEditor;
